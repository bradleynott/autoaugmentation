...
#tf.flags.DEFINE_string('data_path', os.environ['SM_CHANNEL_BATCHES'],
                       #'Directory where dataset is located.')
...
class CifarModel(object):
  ...
  def _setup_images_and_labels(self):
    ...
    # my edit; datasets we will train with
    if FLAGS.dataset in ['cifar10', 'cifar10_10k', 'cifar10_30k', 'cifar102', 'cifar102_30k', 'cifar10_12k', 'cifar105']:
      self.num_classes = 10
...
class CifarModelTrainer(object):
  ...
  def _calc_starting_epoch(self, m):
    ...
    #steps_per_epoch = int(hparams.train_size / batch_size)//len(hparams.hosts)
  ...
  def run_model(self):
    ...
    with tf.Graph().as_default(): # will this pick GPUs better?
...
def main(_):
  #if FLAGS.dataset not in ['cifar10', 'cifar100']: original code
  if FLAGS.dataset not in ['cifar10', 'cifar100', 'cifar10_10k', 'cifar10_30k', 'cifar102', 'cifar102_30k', 'cifar10_12k', 'cifar105']: # edit to accomodate new training sets
    raise ValueError('Invalid dataset: %s' % FLAGS.dataset)
    
  if FLAGS.dataset == 'cifar10':
    training_examples=50000
  elif FLAGS.dataset == 'cifar10_10k':
    training_examples=8000
  elif FLAGS.dataset == 'cifar10_30k':
    training_examples=24000
  elif FLAGS.dataset == 'cifar102': # this is the cifar102_10k dataset
    training_examples=8000
  elif FLAGS.dataset == 'cifar102_30k':
    training_examples=24000
  elif (FLAGS.dataset == 'cifar105') or (FLAGS.dataset == 'cifar10_12k'):
    training_examples=10000
    
  hparams = tf.contrib.training.HParams(
      #train_size=50000, # original code
      train_size=training_examples, # edit toallow for diff training set sizes based on new training sets
      validation_size=0,
      eval_test=1,
  ...
  if FLAGS.model_name == 'wrn':
    ...
    #
    #hparams.add_hparam('hosts', json.loads(os.environ.get('SM_HOSTS')))
    #
