...
def get_lr(curr_epoch, hparams, iteration=None):
  ...
  #batches_per_epoch = int(hparams.train_size / hparams.batch_size)//len(hparams.hosts)
...
def run_epoch_training(session, model, data_loader, curr_epoch):
  ...
  #steps_per_epoch = int(model.hparams.train_size / model.hparams.batch_size)//len(model.hparams.hosts)
